...
class Texttable:
    ...
    # --- gfariello -- End -- Added init with table def.
        ...
        # breaking older code that called Texttable(50) but not
        # Texttable(max_width=50). It felt less intuitive to have the rows
    ...
    def draw(self):
        ...
        out = ""
        if self._has_border():
            out += self._hline(location=Texttable.TOP)
        if self._header:
            out += self._draw_line(self._header, isheader=True)
            if self._has_header():
                out += self._hline_header(location=Texttable.MIDDLE)
                pass
            pass
        num = 0
        ...
        for row in self._rows:
            ...
            if self._has_hlines() and num < length:
                out += self._hline(location=Texttable.MIDDLE)
        if self._has_border():
            out += self._hline(location=Texttable.BOTTOM)
        return out[:-1]
    ...
    def _build_hline(self, is_header=False, location=MIDDLE):
        ...
        if Texttable.TOP == location:
            ...
            if is_header:
                ...
                pass
        elif Texttable.BOTTOM == location:
            ...
            raise ValueError("Unknown location '%s'. Should be one of Texttable.TOP, Texttable.MIDDLE, or Texttable.BOTTOM." %(location))
